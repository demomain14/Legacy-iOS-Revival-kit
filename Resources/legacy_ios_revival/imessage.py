...
def backup_imessage_settings(ssh_target: str | None = None) -> None:
    ...
    run_command(f"scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {ssh_target}:{remote_path} {shlex.quote(str(local_path))}")
...
def apply_imessage_patch(ssh_target: str | None = None, patch_package: str | None = None) -> None:
    ...
    if patch_package is not None:
        ...
        run_command(f"scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {shlex.quote(str(patch_file))} {ssh_target}:/tmp/{patch_file.name}")
        run_command(f"ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {ssh_target} 'dpkg -i /tmp/{patch_file.name} || true'"
                    )
    ...
    for cmd in commands:
        run_command(f"ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {ssh_target} '{cmd}'")
